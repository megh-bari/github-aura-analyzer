...
const Result = () => {
  ...
  const calculateAuraPoints = (profile, repos) => {
    ...
    let points = 0;
    points += Math.min(followers, 5000) / 50; // Followers contribution
    points += totalStars; // Stars contribution
    points += public_repos * 2; // Repositories contribution
    ...
    const headers = GITHUB_TOKEN
      ...
      : {
        ...
        try {
          ...
          const [userResponse, reposResponse, contributionsResponse, yearContributionsResponse] =
            await Promise.all([
              ...
              fetch(`https://api.github.com/users/${username}/events?per_page=1000`, { headers }), // More events for year total
            ]);
    
          if (!userResponse.ok || !reposResponse.ok || !contributionsResponse.ok || !yearContributionsResponse.ok) {
      ...
      const yearTotalContributions = yearContributionsData.filter(
        (event) => {
